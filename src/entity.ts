...
export const print = ({ name, preferences, capacity }: Entity): string =>
  ...
  } prefers: ${preferences.map(({ name: nameValue }) => nameValue).join(", ")}`;
/**
...
export const addPreferences = (entity: Entity, preferences: Entity[]): void => {
  entity.preferences.push(...preferences);
  preferences.forEach((preference, index) => {
    entity.rankTable[preference.name] = index;
  });
};
...
export const removePreference = (entity: Entity, preference: Entity): void => {
  entity.preferences = entity.preferences.filter(
 ...
 */
export const hasPreference = ({ preferences }: Entity): boolean =>
 ...
 */
export const topPreference = ({ preferences }: Entity): Entity =>
  preferences[0];
...
export const getSuccessors = (
  { preferences }: Entity,
  preference: Entity
): Entity[] => preferences.slice(1 + preferences.indexOf(preference));
...
export const rank = ({ rankTable }: Entity, preference: Entity): number =>
  rankTable.hasOwnProperty(preference.name)
    ? rankTable[preference.name]
    : Infinity;
